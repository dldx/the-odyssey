"""Analyze the right block geometry using the optimal slope and projection clustering."""
...
# Filter to right block (x > 500)
right_seats = [s for s in unique_seats if s["x"] > 500]
print(f"Total seats in right block: {len(right_seats)}")

# Find best slope for right block (we expect it to be positive, slanting up to the right)
best_m = 0
...
for m in np.linspace(-0.5, 0.5, 1000):
    ...
    if len(clusters) == 12 and var < min_variance: # we expect 12 detected rows B to N
        min_variance = var
        best_m = m

print(f"Best slope m for right block: {best_m:.4f}")

# Compute projections with the best slope
